...
- Current implemented app is under `app/`: `CMakeLists.txt` builds `src/main.c`, `src/app_channels.c`, `src/bike_config.c`, `src/bike_shell.c`, and `src/bike_state.c`, with headers in `include/`.
...
- `bike_state` implements the initial `UNREGISTERED`, `AVAILABLE`, `RESERVED`, and `IN_USE` transitions plus a 60-second reservation timeout. The `ERROR` fault path, physical button, LED, MQTT, LTE, GNSS, and NVS persistence remain pending.
- Initial Twister/ZTEST coverage exists under `tests/`. Test command shape is `west twister -p native_sim -T bikeshare-firmware/tests`.
