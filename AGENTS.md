...
- Current implemented app is under `app/`: `CMakeLists.txt` builds `src/main.c`, `src/app_channels.c`, `src/bike_config.c`, `src/bike_shell.c`, `src/bike_state.c`, and `src/led_status.c`, with headers in `include/`.
...
- `bike_state` implements the initial `UNREGISTERED`, `AVAILABLE`, `RESERVED`, and `IN_USE` transitions plus a 60-second reservation timeout. `led_status` observes state changes and maps them to LED patterns, using `led0` when available. The `ERROR` fault path, physical button, MQTT, LTE, GNSS, NVS persistence validation, and hardware LED validation remain pending.
- Initial Twister/ZTEST coverage exists under `tests/` for config validation, core state transitions, and LED mapping. Test command shape is `west twister -p native_sim/native/64 -T bikeshare-firmware/tests`.
